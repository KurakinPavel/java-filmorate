...
(
    ...
    CONSTRAINT uq_film_director UNIQUE (FILM_ID, DIRECTOR_ID)
);
...
(
    ...
    USER_ID      INTEGER REFERENCES PUBLIC.USERS (USER_ID) ON DELETE CASCADE,
    ENTITY_ID    INTEGER,
    TIME_STAMP   BIGINT NOT NULL DEFAULT cast(EXTRACT(EPOCH FROM current_timestamp) * 1000 as BIGINT),
    TYPE_ID      INTEGER REFERENCES PUBLIC.EVENT_TYPES (TYPE_ID) ON DELETE CASCADE,
    OPERATION_ID INTEGER REFERENCES PUBLIC.OPERATIONS (OPERATION_ID) ON DELETE CASCADE
);